...
class MatrixToImageWriterEx {
	...
	public static void main(String[] args) {
		...
		String osPathType=ToolOS.getOsPathType();		
        String saveDirAddr="D:\\";        
        File saveDir = new File(saveDirAddr); 
        if (!saveDir.exists()) {
			saveDir.mkdirs();
		}
        String saveFileAddr=saveDirAddr+osPathType+fileName;
        
		ToolBarCode.encode("你好hello", 50, 50, extension.substring(1), saveFileAddr);		
